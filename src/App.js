...
// ── MATCH CARD ─────────────────────────────────────────────────────────────────
function MatchCard({ match, myPred, onSave, isAdmin, isAdminPlayer, onSetResult, allPreds, participants, scoring, champPredictions, tournamentWinner }) {
        ...
        <div style={{ marginTop: 12, padding: "10px 12px", background: "rgba(255,215,0,0.06)", borderRadius: 10, border: "1px solid rgba(255,215,0,0.2)" }}>
          ...
          <div className="reveal-grid">
            {Object.entries(champPredictions).map(([uid, p]) => {
              const hit = tournamentWinner && p.team && p.team.trim().toLowerCase().replace(/\s+/g, "") === tournamentWinner.trim().toLowerCase().replace(/\s+/g, "");
              return (
                <div key={uid} className="reveal-item">
                  <div className="reveal-name" style={hit ? { color: "var(--gold)", fontWeight: 700 } : undefined}>{p.userName}</div>
                  <div className="reveal-score" style={hit ? { color: "var(--gold)", fontWeight: 700 } : undefined}>
                    {TEAM_FLAGS[p.team] || "🏳️"} {p.team}{hit && " 🏆"}
                  </div>
                </div>
              );
            })}
              ...
              <div>
                ...
                {predTab === "upcoming" && (
                  upcomingMatches.length === 0
                    ...
                    : upcomingMatches.map(m => <MatchCard key={m.id} match={m} myPred={predictions[m.id]?.[currentUser?.id]} allPreds={predictions[m.id] || {}} onSave={savePrediction} isAdmin={isAdmin} isAdminPlayer={isAdminPlayer} onSetResult={setResult} participants={participants} scoring={scoring} champPredictions={champPredictions} tournamentWinner={settings.tournamentWinner} />)
                )}
                {predTab === "finished" && (
                  finishedMatches.length === 0
                    ? <div className="empty"><div className="empty-icon">🍃</div><div className="empty-text">Ningún partido finalizado aún</div></div>
                    : finishedMatches.map(m => <MatchCard key={m.id} match={m} myPred={predictions[m.id]?.[currentUser?.id]} allPreds={predictions[m.id] || {}} onSave={savePrediction} isAdmin={isAdmin} isAdminPlayer={isAdminPlayer} onSetResult={setResult} participants={participants} scoring={scoring} champPredictions={champPredictions} tournamentWinner={settings.tournamentWinner} />)
                )}
              </div>
            )}
